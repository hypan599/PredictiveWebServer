...
var metricCount  = [2,1,2,2,6,1,4,5,4,1,7,3,1,2,3,2,1,1,1]
var metricdates = ["11-28","12-02","12-03","12-04","12-05","12-07","12-09","12-10","12-11","12-13","12-14","12-15","12-19","12-20","12-22","12-24","12-25","12-27","12-28"]
var optwidth        = 600;
var optheight       = 370;
...
// Combine the dates and count array to make "data"
var dataset = [];
for(var i=0; i<metricCount.length; i++){
    var obj = {count: metricCount[i], date: metricdates[i]};
    dataset.push(obj);
}

// format date as a date
dataset.forEach(function(d) {
    d.date = d3v3.time.format("%m-%d").parse(d.date);
});

// sort dataset by date
dataset.sort(function(x, y){
   return d3v3.ascending(x.date, y.date);
});
...
var dataXrange = d3v3.extent(dataset, function(d) { return d.date; });
...
var DateFormat	  =  d3v3.time.format("%b %m");

var dynamicDateFormat = timeFormat([
    [d3v3.time.format("%m"), function() { return true; }],// <-- how to display when Jan 1 YYYY
    [d3v3.time.format("%b %m"), function(d) { return d.getDate(); }],
    //[function(){return "";}, function(d) { return d.getDate() != 1; }]
]);

// var dynamicDateFormat =  timeFormat([
//     [d3v3.time.format("%m"), function() { return true; }],
//     [d3v3.time.format("%b"), function(d) { return d.getDate(); }],
//     [function(){return "";}, function(d) { return d.getDate() != 1; }]
// ]);
...
var vis = d3v3.select("#metric-modal").append("svg")
...
// don't show month button if < 1 month of data
var dateRange  = dataXrange[1] - dataXrange[0],
    ms_in_month = 31540000000;

if (dateRange < ms_in_month)   {
    var button_data =["date","data"];
} else {
    var button_data =["month","date","data"];
};
...
function customTickFunction(t0, t1, dt)  {
    ...
    var time = t0, times = [], dateFactors = [1,3,4,30];
    //var time = d3v3.time.date.ceil(t0), times = [], dateFactors = [1,3,4,12];
    ...
    for(i=0 ; times.length > maxTotalLabels ; i++)
        times = _.filter(timesCopy, function(d){
            return (d.getDate()) % dateFactors[i] == 0;
        });
    console.log(times)
    return times;
};
...
// move back to boundaries if user pans outside min and max date.
    ...
    var ms_in_month = 31536000000,
        brush_start_new,
        brush_end_new;

    if       (b[0] < mindate)   { brush_start_new = mindate; }
    else if  (b[0] > maxdate)   { brush_start_new = new Date(maxdate.getTime() - ms_in_month); }
    else                        { brush_start_new = b[0]; };

    if       (b[1] > maxdate)   { brush_end_new = maxdate; }
    else if  (b[1] < mindate)   { brush_end_new = new Date(mindate.getTime() + ms_in_month); }
...
// this function dynamically changes the y-axis to fit the data in focus
    ...
    var bisectDate = d3v3.bisector(function(d) { return d.date; }).right;
...
// action for buttons that scale focus to certain time interval
    ...
    if      (d == "month")   { interval_ms = 31536000000}
    else if (d == "date")  { interval_ms = 2592000000 };
